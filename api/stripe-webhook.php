...
$boundary='=_EternaWeb_'.bin2hex(random_bytes(12));$to='eternaweb.contact@gmail.com';$subject='Nouvelle commande EternaWeb — '.($order['plan_label']??'Commande');$customer=$order['email']??'';
$headers=['From: EternaWeb <eternaweb.contact@gmail.com>','Reply-To: '.$customer,'MIME-Version: 1.0','Content-Type: multipart/mixed; boundary="'.$boundary.'"'];
...
$body.="--$boundary--\r\n";$sent=mail($to,$subject,$body,implode("\r\n",$headers));$order['sent']=$sent;$order['sent_at']=$sent?gmdate('c'):null;file_put_contents($orderFile,json_encode($order,JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE),LOCK_EX);
